...
export default function TeamDetailPage({ params }) {
  ...
  return <main className="fot-shell pb-10"><div className="fot-container space-y-5">
    ...
    <section className="glass card rounded-3xl p-5 flex items-center gap-4"><div className="h-20 w-20 rounded-3xl bg-white/5 grid place-items-center overflow-hidden">{team.logo && <img src={team.logo} alt="" className="h-16 w-16 object-contain"/>}</div><div className="min-w-0"><h2 className="text-2xl font-black truncate">{team.name || "تیم"}</h2><p className="text-xs text-slate-500 mt-1">{team.country || "—"} {team.national ? "· تیم ملی" : ""}</p><p className="text-[10px] text-cyan-300 mt-2">{data.league?.name || "رقابت نامشخص"}</p></div></section>
