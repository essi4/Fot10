...
function Team({ name, logo }) {
  return (
    <div className="flex min-w-0 items-center gap-2">
      <div className="grid h-10 w-10 shrink-0 place-items-center overflow-hidden rounded-2xl bg-white/5 ring-1 ring-white/10">
        {logo ? <img src={logo} alt="" className="h-8 w-8 object-contain" /> : <span className="text-sm font-black">{name?.slice(0, 1) || "?"}</span>}
      </div>
...
function MatchCard({ match }) {
  ...
  return (
    <Link href={`/matches/${match.id}`} className="glass card block w-full p-4 transition-transform active:scale-[.985]">
      <div className="mb-4 flex items-center justify-between gap-2 text-[10px] text-slate-400">
        <span>{match.country || "فوتبال"} • {match.league || "مسابقه"}</span>
        {live ? <span className="rounded-full bg-emerald-400/10 px-2.5 py-1 font-black text-emerald-300">● زنده {match.elapsed ? fa(`${match.elapsed}'`) : ""}</span> : <span>{finished ? "پایان" : fa(time)}</span>}
      </div>
      <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-3">
        <Team name={match.home} logo={match.homeLogo} />
        <div className="min-w-[62px] text-center">
          {match.homeScore != null || match.awayScore != null ? <div className="text-xl font-black">{fa(match.homeScore ?? 0)} - {fa(match.awayScore ?? 0)}</div> : <div className="text-lg font-black">{fa(time)}</div>}
          <div className="mt-1 text-[9px] text-slate-500">{match.status || "برنامه‌ریزی‌شده"}</div>
        </div>
        <div className="justify-self-end"><Team name={match.away} logo={match.awayLogo} /></div>
      </div>
      <div className="mt-4 flex items-center justify-between border-t border-white/5 pt-3 text-[10px] text-slate-500"><span>جزئیات و مرکز مسابقه</span><ChevronLeft size={14} /></div>
    </Link>
  );
}
  ...
  return (
    <main className="fot-container pb-28">
      ...
      <section className="grid grid-cols-3 gap-2">
        <div className="glass rounded-2xl p-3 text-center"><div className="text-lg font-black text-emerald-300">{fa(liveCount)}</div><div className="text-[9px] text-slate-500">زنده</div></div>
        <div className="glass rounded-2xl p-3 text-center"><div className="text-lg font-black">{fa(matches.length)}</div><div className="text-[9px] text-slate-500">بازی امروز</div></div>
        <div className="glass rounded-2xl p-3 text-center"><div className="text-lg font-black">{fa(leagueCount)}</div><div className="text-[9px] text-slate-500">لیگ</div></div>
      </section>

      <section className="mt-5">
        <div className="mb-4 flex items-center justify-between"><div><h1 className="text-xl font-black">مرکز مسابقات</h1><p className="mt-1 text-[10px] text-slate-500">داده زنده و برنامه بازی‌ها از منبع فوتبال</p></div><Activity size={19} className="text-emerald-400" /></div>
        <div className="mb-4 grid grid-cols-3 gap-2">
          {[["دیروز", -1], ["امروز", 0], ["فردا", 1]].map(([label, value]) => <button key={value} onClick={() => setDay(value)} className={`touch-target rounded-2xl px-3 py-3 text-xs font-black ${day === value ? "bg-emerald-400 text-slate-950" : "glass text-slate-400"}`}>{label}</button>)}
        </div>
