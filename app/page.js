...
export default function HomePage() {
  ...
  return <main className="fot-container pb-28"><header className="py-6"><div className="flex items-center justify-between"><Link href="/" className="block"><div className="text-2xl font-black tracking-tight">FOT<span className="text-emerald-400">10</span></div><p className="mt-1 text-[10px] text-slate-500">نبض فوتبال، لحظه‌به‌لحظه</p></Link><button onClick={() => load(true)} disabled={refreshing} className="glass touch-target grid h-11 w-11 place-items-center rounded-2xl" aria-label="به‌روزرسانی"><RefreshCw size={17} className={refreshing ? "animate-spin" : ""} /></button></div></header>
    <Link href="/number10" className="glass card mb-5 block overflow-hidden p-5 active:scale-[.99] transition-transform"><div className="flex items-center gap-4"><div className="grid h-16 w-16 place-items-center rounded-2xl bg-yellow-400/10 text-4xl font-black italic text-yellow-300">۱۰</div><div className="flex-1"><p className="text-[10px] font-black text-yellow-300">امضای FOT10</p><h1 className="text-xl font-black">شماره ۱۰</h1><p className="mt-1 text-[10px] text-slate-500">با انتخاب تیم، خودِ بازیکنِ پیراهن شماره ۱۰ پیدا می‌شود؛ باشگاه یا تیم ملی.</p></div><ChevronLeft className="text-slate-500" size={18}/></div></Link>
