...
function MatchCard({ match, onOpen }) {
  return <button onClick={() => onOpen(match)} className="glass card p-4 w-full text-right active:scale-[.985] transition-transform duration-150">
    ...
    <div className="mt-4 pt-3 border-t border-white/5 flex items-center justify-between text-[11px] text-slate-400"><span>مشاهده مرکز مسابقه</span><ChevronLeft size={15}/></div>
  </button>;
}

function MatchCenter({ match, close }) {
  const events = [["18'", "کارت زرد", match.home], ["31'", "موقعیت خطرناک", match.away], ["45'", "پایان نیمه اول", "بازی"]];
  return <div className="fixed inset-0 z-50 bg-[#060810]/95 backdrop-blur-xl overflow-y-auto overscroll-contain"><div className="fot-container pb-8">
    <button onClick={close} className="glass touch-target rounded-2xl px-4 py-2 text-xs mb-5">← بازگشت</button>
    <div className="glass card p-5 text-center"><div className="text-xs text-slate-400">{match.league}</div><div className="text-[10px] text-emerald-400 font-bold mt-2">● CENTER • DEMO DATA</div><div className="grid grid-cols-3 items-center gap-2 mt-7"><div><div className="mx-auto h-16 w-16 rounded-3xl bg-blue-500/15 text-blue-300 grid place-items-center text-xl font-black">{match.home[0]}</div><div className="font-black mt-3 text-sm">{match.home}</div></div><div><div className="text-3xl font-black">—</div><div className="text-xs text-slate-500 mt-2">{match.time}</div></div><div><div className="mx-auto h-16 w-16 rounded-3xl bg-red-500/15 text-red-300 grid place-items-center text-xl font-black">{match.away[0]}</div><div className="font-black mt-3 text-sm">{match.away}</div></div></div>
      <div className="grid grid-cols-3 gap-2 mt-7"><Stat n="—" t="نتیجه"/><Stat n="0" t="گل"/><Stat n="—" t="xG"/></div>
    </div>
    <div className="glass card p-5 mt-4"><h3 className="font-black mb-5">رویدادهای مسابقه</h3>{events.map(([time,title,team]) => <div key={time} className="flex items-center gap-3 py-3 border-b border-white/5 last:border-0"><span className="text-xs text-emerald-400 font-black w-10">{time}</span><div className="h-2 w-2 shrink-0 rounded-full bg-emerald-400"/><div><div className="text-sm font-bold">{title}</div><div className="text-[10px] text-slate-500 mt-1">{team}</div></div></div>)}</div>
...
export default function HomePage() {
  ...
  const filtered = useMemo(() => matches.filter(m => `${m.home} ${m.away} ${m.league}`.includes(query.trim())), [query]);
  ...
  return <main className="fot-shell">
    <div className="fot-container space-y-5">
      ...
      {tab === "home" && <>
        ...
        <div className="flex gap-2 overflow-x-auto scrollbar-none pb-1">{leagues.map((x,i)=><button key={x} className={`shrink-0 rounded-full px-4 py-2.5 text-xs font-bold border transition ${i===0?'bg-emerald-400 text-slate-950 border-emerald-400':'glass border-white/10 text-slate-300'}`}>{x}</button>)}</div>
        <section><div className="flex items-end justify-between mb-3"><div><h2 className="font-black text-lg">بازی‌های مهم</h2><p className="text-[10px] text-slate-500 mt-1">منتخب FOT10</p></div><span className="text-[10px] text-slate-500">دمو • آماده اتصال API</span></div><div className="space-y-3">{filtered.map(m=><MatchCard key={m.id} match={m} onOpen={setSelected}/>)}</div>{filtered.length===0 && <div className="glass rounded-2xl p-7 text-center text-sm text-slate-500">نتیجه‌ای پیدا نشد</div>}</section>
      </>}

      {tab === "matches" && <section><PageTitle icon={CalendarDays} title="همه بازی‌ها" sub="برنامه مسابقات و نتایج"/><div className="glass card p-4 mb-4"><div className="flex gap-2 overflow-x-auto scrollbar-none"><span className="shrink-0 rounded-xl bg-emerald-400/10 text-emerald-300 px-3 py-2 text-xs font-bold">امروز</span><span className="shrink-0 rounded-xl bg-white/5 text-slate-400 px-3 py-2 text-xs">فردا</span><span className="shrink-0 rounded-xl bg-white/5 text-slate-400 px-3 py-2 text-xs">دیروز</span></div></div><div className="space-y-3">{matches.map(m=><MatchCard key={m.id} match={m} onOpen={setSelected}/>)}</div></section>}

      {tab === "leagues" && <section><PageTitle icon={Trophy} title="لیگ‌ها" sub="رقابت‌های محبوب فوتبال"/><div className="grid grid-cols-2 gap-3">{["لیگ برتر انگلیس","لالیگا","سری آ","بوندسلیگا","لیگ قهرمانان","لیگ برتر ایران"].map((x,i)=><button key={x} className="glass card p-4 text-right active:scale-[.985] transition"><div className="h-11 w-11 rounded-2xl bg-emerald-400/10 text-emerald-300 grid place-items-center mb-4"><Trophy size={21}/></div><div className="font-black text-sm">{x}</div><div className="text-[10px] text-slate-500 mt-1">جدول • بازی‌ها • اخبار</div></button>)}</div></section>}
      ...
      <section className="glass card p-4 flex items-center gap-3"><Shield size={19} className="text-emerald-400 shrink-0"/><div><div className="text-xs font-black">FOT10 در حال ساخته‌شدن است</div><div className="text-[10px] text-slate-500 mt-1 leading-5">داده‌های نمایشی هستند؛ اتصال به منبع معتبر در مرحله بعد انجام می‌شود.</div></div></section>
    </div>

    <nav className="fixed bottom-0 inset-x-0 z-20 border-t border-white/10 bg-[#080b14]/90 backdrop-blur-xl safe-bottom"><div className="mx-auto max-w-[760px] grid grid-cols-4 px-3 pt-2 pb-1">{[["خانه",Home,"home"],["بازی‌ها",Tv,"matches"],["لیگ‌ها",Trophy,"leagues"],["جستجو",Search,"search"]].map(([label,Icon,id])=><button key={id} onClick={()=>setTab(id)} className={`touch-target rounded-2xl flex flex-col items-center justify-center gap-1 transition ${tab===id?'text-emerald-400 bg-emerald-400/5':'text-slate-500'}`}><Icon size={20}/><span className="font-bold text-[10px]">{label}</span></button>)}</div></nav>
