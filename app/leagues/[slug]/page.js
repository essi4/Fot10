...
import { getLeagueByCountry, getLeagueCurrentSeason, getMatches, getStandings } from "../../../lib/sports-data";
...
export default async function LeagueDetailPage({ params }) {
  ...
  let matches = [], standings = [], season = null, liveError = null, resolved = null;
  try {
    ...
    [matches, standings] = await Promise.all([getMatches({ date: today, league: leagueId, season }), getStandings(leagueId, season)]);
  ...
  return <main className="fot-shell"><div className="fot-container space-y-5">
    ...
    {liveError && <div className="glass rounded-2xl p-4 text-center text-xs text-amber-300">دریافت اطلاعات زنده این رقابت فعلاً موفق نشد؛ صفحه آماده است و با برگشت منبع داده دوباره تلاش می‌کند.</div>}
    ...
    <section className="space-y-2"><h2 className="flex items-center gap-2 text-sm font-black"><Trophy size={16}/> جدول</h2>{rows.length ? <div className="glass rounded-2xl overflow-hidden">{rows.slice(0, 10).map((row) => <div key={row.team?.id} className="px-4 py-3 border-b border-white/5 flex items-center gap-3 text-xs"><b className="w-5">{row.rank}</b><span className="flex-1">{row.team?.name}</span><span>{row.points} امتیاز</span></div>)}</div> : <div className="glass rounded-2xl p-5 text-center text-sm text-slate-500">جدول این رقابت فعلاً داده‌ای برنگرداند.</div>}</section>
  </div></main>;
}
