...
function errorInfo(result) {
  ...
  return {
    ...
    message: error?.message || "درخواست ناموفق بود.",
    status: error?.details?.status || null,
    dailyRemaining: error?.details?.dailyRemaining || null,
  };
}
...
export default async function LeagueDetailPage({ params, searchParams }) {
  ...
  try {
    ...
    diagnostics = results.map((result, index) => ({
      name: ["بازی‌ها", "جدول", "گلزنان", "پاس گل"][index],
      ok: result.status === "fulfilled",
      count: result.status === "fulfilled" ? result.value?.length || 0 : 0,
