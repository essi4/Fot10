...
function RecentMatches({ teamId, leagueId, season }) {
  ...
  const load = useCallback(async () => {
    ...
    } catch (err) { setError(err?.message || "فرم تیم در دسترس نیست."); }
  ...
  return <div className="relative flex justify-center">
    ...
    {open && <div className="absolute right-1/2 top-[calc(100%+6px)] z-30 w-[270px] translate-x-1/2 rounded-2xl border border-white/10 bg-slate-950 p-3 shadow-2xl">
      {loading ? <div className="py-2 text-center text-[9px] text-slate-500">در حال دریافت ۵ بازی اخیر…</div> : error ? <div className="py-2 text-center text-[9px] text-rose-300">{error}</div> : !matches.length ? <div className="py-2 text-center text-[9px] text-slate-500">نتیجه‌ای برای این تیم پیدا نشد.</div> : <div className="grid gap-1.5">
        {matches.map(match => <div key={`${match.id}-${match.date}`} className="flex items-center gap-2 rounded-xl border border-white/[.07] bg-white/[.025] px-2 py-1.5">
          <span className={`grid h-5 w-5 shrink-0 place-items-center rounded-full text-[8px] font-black ${match.result === "W" ? "bg-emerald-400/20 text-emerald-300" : match.result === "D" ? "bg-amber-400/20 text-amber-300" : "bg-rose-400/20 text-rose-300"}`}>{match.result}</span>
          <span className="min-w-0 flex-1 truncate text-[8px] font-bold">{match.home} - {match.away}</span>
          <b className="shrink-0 text-[9px] text-slate-300">{match.homeScore}-{match.awayScore}</b>
          <span className="shrink-0 text-[7px] text-slate-600">{match.date ? new Date(match.date).toLocaleDateString("fa-IR", { month: "short", day: "numeric" }) : "—"}</span>
        </div>)}
      </div>}
    </div>}
  </div>;
...
export default function LeagueStandingsLive({ leagueId, leagueName, country, season = SEASON }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setRows(payload.data); setMeta({ provider: payload.provider || "unknown", updatedAt: new Date(), loading: false, error: null });
    } catch (error) { setMeta(current => ({ ...current, loading: false, error: error?.message || "دریافت جدول ناموفق بود." })); }
  }, [leagueId, season]);
  ...
  return <main className="fot-shell min-h-screen pb-12">
    <div className="fot-container space-y-4">
      <header className="glass overflow-hidden rounded-[26px] border border-white/10"><div className="bg-gradient-to-l from-cyan-400/[.12] via-emerald-400/[.05] to-transparent p-5">
      ...
      <section className="glass overflow-hidden rounded-[24px] border border-white/10"><div className="overflow-x-auto">
        <table className="w-full min-w-[700px] border-collapse text-[10px] sm:text-[11px]"><thead><tr className="bg-white/[.025] text-slate-500"><th className="w-10 p-3">#</th><th className="sticky right-0 bg-slate-950/95 p-3 text-right">تیم</th><th className="p-3">بازی</th><th className="p-3">برد</th><th className="p-3">مساوی</th><th className="p-3">باخت</th><th className="p-3">گل</th><th className="p-3">تفاضل</th><th className="p-3">فرم</th><th className="p-3">امتیاز</th></tr></thead><tbody>
          {rows.map((row, index) => {
            const name = row?.team?.name || "—"; const id = row?.team?.id; const gd = Number(row?.goalsDiff || 0);
            return <tr key={`${id || name}-${index}`} className="border-t border-white/[.055] hover:bg-cyan-400/[.035]"><td className="p-2.5 text-center font-black text-slate-500">{row?.rank || index + 1}</td><td className="sticky right-0 bg-slate-950/95 p-2.5"><Link href={id ? `/teams/${id}` : `/teams?search=${encodeURIComponent(name)}`} className="flex min-w-[170px] items-center gap-2 font-bold"><span className="grid h-8 w-8 shrink-0 place-items-center overflow-hidden rounded-lg border border-white/10 bg-white/[.05]">{row?.team?.logo ? <img src={row.team.logo} alt="" className="h-7 w-7 object-contain" /> : "⚽"}</span><span className="truncate">{name}</span></Link></td><td className="p-2.5 text-center">{row?.all?.played ?? 0}</td><td className="p-2.5 text-center text-emerald-300">{row?.all?.win ?? 0}</td><td className="p-2.5 text-center text-amber-300">{row?.all?.draw ?? 0}</td><td className="p-2.5 text-center text-rose-300">{row?.all?.lose ?? row?.all?.loss ?? 0}</td><td className="p-2.5 text-center">{row?.all?.goals?.for ?? 0}-{row?.all?.goals?.against ?? 0}</td><td className="p-2.5 text-center font-black">{gd > 0 ? `+${gd}` : gd}</td><td className="p-2"><RecentMatches teamId={id} leagueId={leagueId} season={season} /></td><td className="p-2.5 text-center text-sm font-black text-cyan-200">{row?.points ?? 0}</td></tr>;
          })}
        </tbody></table>
      ...
      <p className="px-2 text-center text-[8px] leading-5 text-slate-600">فرم تیم با باز کردن «۵ بازی» از نتایج واقعی همان تیم دریافت می‌شود؛ جدول نیز خودکار بروزرسانی می‌شود.</p>
    </div>
  </main>;
}
