...
const LIVE_STATUSES = ["1H", "HT", "2H", "ET", "P", "LIVE", "IN PLAY"];
...
const cleanTeamName = n => String(n || "—").replace(/\s*-\s*e\s+Babak$/i, "").replace(/\s*-\s*Khuzestan$/i, " خوزستان").replace(/\s+Khuzestan$/i, " خوزستان").replace(/\s+/g, " ").trim();
const leagueLogo = id => id ? `https://media.api-sports.io/football/leagues/${id}.png` : null;
const teamLogo = id => id && !String(id).startsWith("iran-") ? `https://media.api-sports.io/football/teams/${id}.png` : null;
function resultForTeam(m, team, teamId) {
  const id = String(teamId || "");
  const homeId = String(m?.homeId || "");
  const awayId = String(m?.awayId || "");
  const t = norm(cleanTeamName(team));
  const h = norm(cleanTeamName(m?.home));
  const a = norm(cleanTeamName(m?.away));
  const isHome = (id && id === homeId) || (!id && t === h) || (id && id !== homeId && id !== awayId && t === h);
  const isAway = (id && id === awayId) || (!id && t === a) || (id && id !== homeId && id !== awayId && t === a);
  if (!isHome && !isAway) return null;
  const hs = Number(m?.homeScore), as = Number(m?.awayScore);
  if (!Number.isFinite(hs) || !Number.isFinite(as)) return null;
  return hs === as ? "D" : (isHome ? hs > as : as > hs) ? "W" : "L";
}
...
export default async function League360Page({ params, searchParams }) {
  ...
  let today = [], seasonMatches = [], standings = [], scorers = [], assists = [], liveNow = [], season = null, resolved = null, warning = null;
  try {
    ...
    if (rs[5].status === "fulfilled") liveNow = rs[5].value.data || [];
    const liveForLeague = liveNow.filter(m => String(m?.leagueId || "") === String(id) || norm(m?.league) === norm(league.leagueName));
    if (liveForLeague.length) {
      const merged = new Map(seasonMatches.filter(m => m?.id).map(m => [String(m.id), m]));
      liveForLeague.forEach(m => merged.set(String(m.id), m));
      seasonMatches = [...merged.values()];
    }
  ...
  const upcoming = seasonMatches.filter(m => m?.date && new Date(m.date) > new Date() && !FINISHED.includes(m?.statusShort)).sort((a,b) => new Date(a.date) - new Date(b.date));
  const live = seasonMatches.filter(m => LIVE_STATUSES.includes(String(m?.statusShort || "").toUpperCase()));
  const recentFinished = finished.slice(0, 12);
  const logoMap = new Map();
  seasonMatches.forEach(m => {
    if (m?.home && m?.homeLogo) logoMap.set(norm(cleanTeamName(m.home)), m.homeLogo);
    if (m?.away && m?.awayLogo) logoMap.set(norm(cleanTeamName(m.away)), m.awayLogo);
  });
  const getLogo = (name,id) => logoMap.get(norm(cleanTeamName(name))) || teamLogo(id);
  const teamHref = r => r?.team?.id && !String(r.team.id).startsWith("iran-") ? `/teams/${r.team.id}` : `/teams?search=${encodeURIComponent(cleanTeamName(r?.team?.name || ""))}`;
  const form = r => finished.filter(m => resultForTeam(m, r?.team?.name, r?.team?.id)).slice(0,5).map(m => resultForTeam(m, r?.team?.name, r?.team?.id));
  ...
  [...live, ...today, ...upcoming, ...recentFinished].forEach(m => { if (m?.id && !matchMap.has(String(m.id))) matchMap.set(String(m.id), m); });
