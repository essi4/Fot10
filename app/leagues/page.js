...
import { ArrowRight, ChevronLeft, Crown, Flag, Search, SlidersHorizontal, Trophy, X, Sparkles, Globe2, Flame, CalendarDays } from "lucide-react";
...
function LeagueCard({ league, featured = false }) {
  return (
    <Link href={`/leagues/${league.slug}`} className={`group relative overflow-hidden rounded-[24px] border border-white/[.07] text-right transition duration-200 active:scale-[.985] hover:-translate-y-0.5 ${featured ? "min-h-[126px] bg-[radial-gradient(circle_at_85%_15%,rgba(16,185,129,.18),transparent_38%),linear-gradient(145deg,#111c29,#070c15)] p-4" : "bg-white/[.035] p-3.5"}`}>
    ...
    <main className="fot-shell min-h-screen bg-[radial-gradient(circle_at_50%_-5%,rgba(16,185,129,.14),transparent_30%),#020617]">
      <div className="fot-container space-y-5 pb-28">
        <header className="flex items-center gap-3 pt-1">
          ...
          <button type="button" aria-label="فیلترها" className="glass grid h-11 w-11 shrink-0 place-items-center rounded-2xl"><SlidersHorizontal size={18} className="text-emerald-400" /></button>
        </header>
        ...
        <div className="relative"><Search className="absolute right-4 top-3.5 text-slate-500" size={18} /><input value={query} onChange={(event) => setQuery(event.target.value)} className="glass w-full rounded-2xl py-3.5 pl-11 pr-11 outline-none placeholder:text-slate-600" placeholder="جستجوی لیگ، کشور یا رقابت..." /><button type="button" onClick={() => setQuery("")} className={`absolute left-3 top-2.5 grid h-8 w-8 place-items-center rounded-xl text-slate-500 ${query ? "" : "hidden"}`} aria-label="پاک کردن جستجو"><X size={15} /></button></div>
        ...
        <section className="space-y-3"><div className="flex items-center justify-between px-1"><div><p className="text-[9px] font-black text-yellow-400">CONTINENTAL CUPS</p><h2 className="mt-1 text-sm font-black">🏆 جام‌های باشگاهی</h2></div><span className="text-[9px] text-slate-500">{CLUB_CUPS.length} رقابت</span></div><div className="grid grid-cols-1 gap-2">{CLUB_CUPS.map((cup) => <Link key={cup.slug} href={`/leagues/${cup.slug}`} className="glass group flex items-center gap-3 rounded-2xl p-4"><span className="grid h-11 w-11 place-items-center rounded-2xl bg-yellow-400/10 text-2xl">{cup.icon}</span><span className="flex-1"><b className="block text-sm">{cup.name}</b><small className="text-[10px] text-slate-500">{cup.country}</small></span><ChevronLeft size={16} className="text-slate-600 transition group-hover:text-yellow-400" /></Link>)}</div></section>
      </div>
    </main>
  );
}
