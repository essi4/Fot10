...
export default function FavoritesPage() {
  ...
  useEffect(() => {
    try {
      ...
      setTeams(Array.isArray(raw.favorites) ? raw.favorites : []);
      setLeagues(Array.isArray(raw.favoriteLeagues) ? raw.favoriteLeagues : []);
    } catch {}
    setReady(true);
  }, []);
  ...
  const removeTeam = (name) => {
    ...
    try {
      const raw = JSON.parse(localStorage.getItem("fot10-profile") || "{}");
      localStorage.setItem("fot10-profile", JSON.stringify({ ...raw, favorites: next }));
    } catch {}
  };
  ...
  const removeLeague = (name) => {
    ...
    try {
      const raw = JSON.parse(localStorage.getItem("fot10-profile") || "{}");
      localStorage.setItem("fot10-profile", JSON.stringify({ ...raw, favoriteLeagues: next }));
    } catch {}
  };
  ...
  return <main className="fot-shell min-h-screen"><div className="fot-container space-y-4 pb-10">
    ...
    <div className="glass rounded-2xl p-4 text-[10px] leading-5 text-slate-600">حالت مهمان روی همین گوشی ذخیره می‌شود. پس از ورود به حساب، علاقه‌مندی‌ها برای همگام‌سازی ابری FOT10 استفاده می‌شوند.</div>
  </div></main>;
}
