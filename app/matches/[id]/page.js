...
import { Activity, ArrowRight, BarChart3, MapPin, RefreshCw, Shield, Users, WifiOff, Zap } from "lucide-react";
...
export default function MatchDetailPage({ params }) {
  ...
  const load = useCallback(async (nextSection = section, silent = false) => {
    ...
    try {
      ...
      if (payload.ok) {
        ...
        if (nextSection === "details") setFixture(null);
        else setData([]);
      }
    } catch {
      setDegraded(true);
  ...
  useEffect(() => { if (section !== "details") load(section); }, [section, id, load]);
  ...
  return (
    <main className="fot-shell">
      <div className="fot-container space-y-4 pb-10">
        ...
        {degraded && <FallbackBanner onRetry={() => load(section === "details" ? "details" : section)} />}
        ...
        <section className="space-y-3">
          {loading ? <div className="glass rounded-2xl p-8 text-center text-sm text-slate-400"><Activity className="mx-auto mb-2 animate-pulse" size={20} />در حال بارگذاری…</div> : section === "events" ? <Events data={data} /> : section === "statistics" ? <Statistics data={data} /> : section === "lineups" ? <Lineups data={data} /> : section === "players" ? <Players data={data} /> : <Details data={fixture} />}
        </section>
      </div>
    </main>
  );
...
function FallbackBanner({ onRetry }) {
  return <section className="glass rounded-2xl border border-amber-300/10 bg-amber-300/[.035] p-4 md:p-5"><div className="flex items-start gap-3"><div className="h-10 w-10 shrink-0 rounded-xl bg-amber-300/10 grid place-items-center"><WifiOff size={18} className="text-amber-300" /></div><div className="min-w-0 flex-1"><p className="text-[10px] font-black text-amber-300">حالت پایدار FOT10</p><h2 className="mt-1 text-sm font-black">سرویس داده فوتبال فعلاً در دسترس نیست</h2><p className="mt-1 text-[10px] leading-5 text-slate-500">هیچ نتیجه یا رویداد ساختگی نمایش نمی‌دهیم. به محض برقرار شدن سرویس، اطلاعات واقعی دوباره دریافت می‌شود.</p></div><button onClick={onRetry} className="shrink-0 rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-[9px] font-bold text-slate-300 hover:bg-white/10"><RefreshCw size={13} className="mx-auto mb-1" />تلاش مجدد</button></div></section>;
}
...
function TeamHero({ team }) {
  ...
  return <div className="text-center min-w-0"><Link href={`/teams/${team.id}`} className="inline-flex h-20 w-20 md:h-24 md:w-24 items-center justify-center rounded-3xl bg-white/5 border border-white/5"><img src={team.logo} alt="" className="h-14 w-14 md:h-16 md:w-16 object-contain" /></Link><Link href={`/teams/${team.id}`} className="block mt-3 font-black text-sm md:text-base truncate hover:text-emerald-300">{team.name || "تیم"}</Link></div>;
}

function Empty({ text, retry }) { return <div className="glass rounded-2xl p-8 text-center"><div className="mx-auto h-11 w-11 rounded-xl bg-white/5 grid place-items-center"><WifiOff size={18} className="text-slate-600" /></div><p className="mt-3 text-xs text-slate-500">{text}</p>{retry && <button onClick={retry} className="mt-3 rounded-xl bg-white/5 px-4 py-2 text-[9px] font-bold text-slate-400">تلاش مجدد</button>}</div>; }

function Events({ data }) { if (!Array.isArray(data) || !data.length) return <Empty text="هنوز رویدادی برای این مسابقه ثبت نشده است." />; return <div className="glass rounded-2xl p-3 md:p-5">{data.map((e, i) => { const minute = `${e.time?.elapsed ?? "—"}${e.time?.extra ? `+${e.time.extra}` : ""}'`; return <div key={`${e.time?.elapsed}-${e.time?.extra}-${i}`} className="grid grid-cols-[1fr_46px_1fr] items-center gap-2 py-3 border-b border-white/5 last:border-0"><div className="text-right"><b className="text-xs">{e.team?.id ? <Link href={`/teams/${e.team.id}`} className="hover:text-emerald-300">{e.team.name}</Link> : ""}</b><div className="text-[11px] text-slate-300">{e.player?.id ? <Link href={`/players/${e.player.id}`} className="hover:text-emerald-300">{e.player.name}</Link> : e.player?.name || ""}</div>{e.assist?.name && <div className="text-[9px] text-slate-500">پاس گل: {e.assist.name}</div>}</div><div className="text-center"><div className="text-lg">{eventIcon(e)}</div><div className="text-[10px] font-black text-emerald-300">{minute}</div></div><div className="text-right"><b className="text-[10px] text-slate-500">{eventTitle(e)}</b></div></div>; })}</div>; }

function Statistics({ data }) { if (!Array.isArray(data) || !data.length) return <Empty text="آمار این مسابقه هنوز در دسترس نیست." />; const home = data[0]?.statistics || []; const away = data[1]?.statistics || []; const awayMap = new Map(away.map((item) => [String(item.type).toLowerCase(), item.value])); return <div className="glass rounded-2xl p-4 md:p-5 space-y-1">{home.map((item, i) => { const left = item.value ?? "—"; const right = awayMap.get(String(item.type).toLowerCase()) ?? "—"; const percentages = typeof left === "string" && left.endsWith("%") && typeof right === "string" && right.endsWith("%"); const lp = percentages ? Math.max(0, Math.min(100, Number.parseFloat(left))) : 0; const rp = percentages ? Math.max(0, Math.min(100, Number.parseFloat(right))) : 0; return <div key={item.type || i} className="py-3 border-b border-white/5 last:border-0"><div className="grid grid-cols-[1fr_100px_1fr] items-center gap-2"><span className="text-center font-black text-xs">{left}</span><span className="text-center text-[10px] text-slate-500">{item.type}</span><span className="text-center font-black text-xs">{right}</span></div>{percentages && <div className="mt-2 grid grid-cols-2 gap-2"><div className="h-1.5 rounded-full bg-emerald-400/80" style={{ width: `${lp}%` }} /><div className="h-1.5 rounded-full bg-slate-600/80 ml-auto" style={{ width: `${rp}%` }} /></div>}</div>; })}</div>; }

function Lineups({ data }) { if (!Array.isArray(data) || !data.length) return <Empty text="ترکیب رسمی هنوز اعلام نشده است." />; return <div className="grid gap-3 md:grid-cols-2">{data.map((team, i) => <div key={team.team?.id || i} className="glass rounded-2xl p-4"><div className="flex items-center gap-2 mb-4"><Link href={`/teams/${team.team?.id}`}><img src={team.team?.logo} alt="" className="h-8 w-8 object-contain" /></Link><Link href={`/teams/${team.team?.id}`} className="font-black hover:text-emerald-300">{team.team?.name}</Link><span className="mr-auto rounded-lg bg-white/5 px-2 py-1 text-[10px] text-slate-500">{team.formation || "—"}</span></div><div className="text-[10px] font-black text-emerald-300 mb-2">ترکیب اصلی</div><div className="grid grid-cols-2 gap-2">{(team.startXI || []).map((p, j) => <div key={p.player?.id || j} className="rounded-xl bg-white/5 p-2.5 text-[10px] flex justify-between gap-2"><Link href={p.player?.id ? `/players/${p.player.id}` : "#"} className="truncate hover:text-emerald-300">{p.player?.name}</Link><span className="shrink-0 text-slate-500">{p.player?.number ?? ""}</span></div>)}</div><div className="text-[10px] font-black text-slate-300 mt-4 mb-2">نیمکت</div>{(team.substitutes || []).length ? <div className="grid grid-cols-2 gap-2">{team.substitutes.map((p, j) => <div key={p.player?.id || `sub-${j}`} className="rounded-xl bg-white/5 p-2.5 text-[10px] flex justify-between gap-2"><Link href={p.player?.id ? `/players/${p.player.id}` : "#"} className="truncate hover:text-emerald-300">{p.player?.name}</Link><span className="shrink-0 text-slate-500">{p.player?.number ?? ""}</span></div>)}</div> : <div className="rounded-xl bg-white/5 p-3 text-[10px] text-slate-500">اطلاعات نیمکت هنوز در دسترس نیست.</div>}</div>)}</div>; }

function Players({ data }) { if (!Array.isArray(data) || !data.length) return <Empty text="امتیازات بازیکنان هنوز در دسترس نیست." />; const players = data.flatMap((t) => (t.players || []).map((p) => ({ ...p.player, statistics: p.statistics?.[0], team: t.team }))); return <div className="grid gap-2">{players.map((p, i) => <Link key={p.id || i} href={p.id ? `/players/${p.id}` : "#"} className="glass rounded-2xl p-3 flex items-center gap-3"><img src={p.photo} alt="" className="h-10 w-10 rounded-full object-cover bg-white/5" /><div className="min-w-0 flex-1"><b className="block text-xs truncate">{p.name || "بازیکن"}</b><span className="text-[9px] text-slate-500">{p.team?.name || "—"}</span></div><span className="text-[10px] font-black text-emerald-300">{p.statistics?.games?.rating ? Number(p.statistics.games.rating).toFixed(1) : "—"}</span></Link>)}</div>; }

function Details({ data }) { const match = data?.fixture ? data : null; if (!match) return <Empty text="اطلاعات تکمیلی مسابقه فعلاً در دسترس نیست." />; const rows = [["تاریخ و زمان", formatDate(match.fixture?.date)], ["ورزشگاه", match.fixture?.venue?.name || "—"], ["شهر", match.fixture?.venue?.city || "—"], ["داور", match.fixture?.referee || "—"], ["وضعیت", statusLabel(match.fixture?.status?.short)]]; return <div className="glass rounded-2xl p-4 md:p-5">{rows.map(([label, value]) => <div key={label} className="flex items-center justify-between gap-4 py-3 border-b border-white/5 last:border-0"><span className="text-[10px] text-slate-500">{label}</span><b className="text-xs text-right">{value}</b></div>)}</div>; }
