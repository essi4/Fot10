...
function stats(matches) {
  ...
  const leagues = new Set(rows.map((match) => `${match?.country || ""}|${match?.league || "مسابقات"}`));
  const finished = rows.filter((match) => /^(FT|AET|PEN)$/i.test(String(match?.statusShort || match?.status || ""))).length;
  const goals = rows.reduce((sum, match) => sum + scoreOf(match) + scoreOf({ scoreHome: match?.awayScore, score: { home: match?.score?.away } }), 0);
  return { count: rows.length, leagues: leagues.size, finished, goals };
}
...
export default function MatchCenterGuard() {
  useEffect(() => {
    ...
    const run = async () => {
      ...
      const jobs = cards.map(async (card) => {
        ...
        try {
          ...
          const extra = label === "فردا" ? `${value.leagues} لیگ · برنامه` : `${value.leagues} لیگ · ${value.goals} گل`;
      ...
      if (liveCard) {
        jobs.push((async () => {
          try {
            ...
            if (cancelled) return;
            const value = stats(payload.matches);
            applyCard(liveCard, value.count ? `${value.count} بازی` : "بدون بازی", `${value.leagues} لیگ`);
          } catch {}
        })());
      }
    ...
    const observer = new MutationObserver(() => {
      if (!cancelled) window.setTimeout(run, 100);
    });
