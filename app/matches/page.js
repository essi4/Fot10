...
export default function MatchesPage() {
  ...
  const visibleGames = useMemo(() => filterMajorLeagues(games), [games]);
  const liveCount = visibleGames.filter((g) => g.live).length;
  const grouped = visibleGames.reduce((acc, g) => {
  ...
  return <main className="fot-shell"><div className="fot-container space-y-5">
    ...
    <div className="grid grid-cols-3 gap-2"><div className="rounded-2xl bg-emerald-400 text-slate-950 p-3 text-center"><Radio size={16} className="mx-auto mb-1"/><b className="text-sm">زنده</b><div className="text-[10px]">{liveCount} بازی</div></div><div className="glass rounded-2xl p-3 text-center"><Clock3 size={16} className="mx-auto mb-1 text-slate-400"/><b className="text-sm">{dayLabel}</b><div className="text-[10px] text-slate-500">{visibleGames.length} بازی</div></div><div className="glass rounded-2xl p-3 text-center"><Trophy size={16} className="mx-auto mb-1 text-slate-400"/><b className="text-sm">لیگ‌ها</b><div className="text-[10px] text-slate-500">{Object.keys(grouped).length}</div></div></div>
