...
import { ArrowRight, CalendarDays, ChevronLeft, Clock3, Radio, RefreshCw, Star, Trophy } from "lucide-react";
...
function iranDate(offset = 0) {
  const now = new Date();
  const tehran = new Intl.DateTimeFormat("en-CA", { timeZone: "Asia/Tehran", year: "numeric", month: "2-digit", day: "2-digit" }).formatToParts(now).reduce((a, p) => ({ ...a, [p.type]: p.value }), {});
  const d = new Date(`${tehran.year}-${tehran.month}-${tehran.day}T12:00:00+03:30`);
...
function mapGame(match) {
  ...
  return { ...match, id: match.id, league: match.league || "مسابقات فوتبال", home: match.home || "میزبان", away: match.away || "مهمان", minute: live && match.minute ? `${match.minute}'` : finished ? "پایان" : toTime(match.time), status: live ? "LIVE" : finished ? "FT" : "امروز", live };
}
...
export default function MatchesPage() {
  ...
  async function loadMatches({ manual = false } = {}) {
    ...
    try {
      ...
      setGames(payload.matches.map(mapGame)); setDemo(false);
    } catch (err) {
      setGames(day === 0 ? demoGames : []); setDemo(day === 0); setError("داده زنده فعلاً در دسترس نیست؛ نمایش دمو فقط برای امروز فعال شد.");
  ...
  return <main className="fot-shell"><div className="fot-container space-y-5">
    <header className="flex items-center justify-between"><div className="flex items-center gap-3"><Link href="/" className="glass h-10 w-10 rounded-xl grid place-items-center"><ArrowRight size={19}/></Link><div><h1 className="text-xl font-black">مرکز بازی‌ها</h1><p className="text-[11px] text-slate-500">نتایج و مسابقات واقعی</p></div></div><button onClick={() => loadMatches({ manual: true })} className="glass h-10 w-10 rounded-xl grid place-items-center" aria-label="به‌روزرسانی"><RefreshCw size={18} className={refreshing ? "animate-spin" : ""}/></button></header>
    ...
    {loading ? <section className="glass card p-8 text-center text-sm text-slate-400">در حال دریافت مسابقات واقعی…</section> : Object.keys(grouped).length ? <section className="space-y-4">{Object.entries(grouped).map(([league, list]) => <div key={league}><div className="px-1 mb-2 text-xs font-black text-slate-400">{league}</div><div className="space-y-3">{list.map(g => <Link key={g.id} href={`/matches/${g.id}`} className="glass card block p-4 overflow-hidden active:scale-[.99] transition"><div className="flex justify-between items-center mb-4"><span className="text-[11px] text-slate-400">{g.league}</span>{g.live?<span className="flex items-center gap-1 text-[10px] text-emerald-400 font-black"><i className="h-1.5 w-1.5 rounded-full bg-emerald-400 animate-pulse"/> LIVE</span>:<span className="text-[10px] text-slate-500">{g.minute}</span>}</div><div className="grid grid-cols-[1fr_auto_1fr] items-center gap-3"><div className="text-center"><div className="mx-auto h-11 w-11 rounded-2xl bg-white/5 grid place-items-center overflow-hidden">{g.homeLogo ? <img src={g.homeLogo} alt="" className="h-8 w-8 object-contain"/> : <span className="font-black">{g.home[0]}</span>}</div><div className="mt-2 font-bold text-sm">{g.home}</div></div><div className="text-center"><div className={`text-xl font-black ${g.live?'text-white':'text-slate-300'}`}>{g.homeScore != null ? `${g.homeScore} - ${g.awayScore}` : "—"}</div>{g.live&&<div className="text-[10px] text-emerald-400 mt-1">{g.minute}</div>}</div><div className="text-center"><div className="mx-auto h-11 w-11 rounded-2xl bg-white/5 grid place-items-center overflow-hidden">{g.awayLogo ? <img src={g.awayLogo} alt="" className="h-8 w-8 object-contain"/> : <span className="font-black">{g.away[0]}</span>}</div><div className="mt-2 font-bold text-sm">{g.away}</div></div></div><div className="mt-4 pt-3 border-t border-white/5 flex justify-between items-center text-[10px] text-slate-500"><span>{g.live ? "ورود به گزارش زنده و آمار" : "ورود به مرکز مسابقه"}</span><Star size={14}/></div></Link>)}</div></div>)}</section> : <div className="glass rounded-2xl p-8 text-center text-sm text-slate-500">برای این روز مسابقه‌ای پیدا نشد.</div>}
  </div></main>;
}
