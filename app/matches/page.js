...
function DayFilters({ liveOnly, day, summary }) {
  ...
  return <section className="rounded-[26px] border border-white/10 bg-white/[.025] p-2 shadow-[0_18px_55px_rgba(0,0,0,.18)]"><div className="mb-2 flex items-center justify-between px-2"><div><p className="text-[9px] font-bold text-slate-600">MATCH CENTER</p><h2 className="text-xs font-black text-slate-300">روز مسابقات</h2></div><span className="rounded-full border border-white/10 px-2 py-1 text-[8px] text-slate-500">{formatProjectDate(active === "yesterday" ? projectDate(-1) : active === "tomorrow" ? projectDate(1) : projectDate(0))}</span></div><div className="grid grid-cols-4 gap-1.5">{items.map(({ key, label, date, meta, extra, href, icon: Icon }) => { const isActive = active === key; const isLive = key === "live"; return <Link key={key} href={href} className={`group relative overflow-hidden rounded-[20px] border p-2.5 text-center transition duration-200 active:scale-[.97] ${isActive ? isLive ? "border-red-400/35 bg-gradient-to-b from-red-500/90 to-red-600/75 text-white shadow-[0_10px_30px_rgba(239,68,68,.2)]" : "border-emerald-300/30 bg-gradient-to-b from-emerald-300 to-emerald-400 text-slate-950 shadow-[0_10px_30px_rgba(52,211,153,.15)]" : "border-white/7 bg-white/[.025] text-slate-400 hover:bg-white/[.05]"}`}>{isActive && <span className={`absolute inset-x-4 top-0 h-px ${isLive ? "bg-red-100/70" : "bg-white/70"}`} />}<span className={`mx-auto mb-1.5 grid h-7 w-7 place-items-center rounded-xl ${isActive ? isLive ? "bg-white/15" : "bg-slate-950/10" : "bg-white/5"}`}>{isLive ? <span className="relative"><Icon size={15}/><i className="absolute -right-1 -top-1 h-2 w-2 rounded-full bg-red-300 shadow-[0_0_0_3px_rgba(248,113,113,.12)] animate-pulse"/></span> : <Icon size={15}/>}</span><b className="block text-[10px]">{label}</b><span className={`mt-0.5 block text-[8px] ${isActive ? isLive ? "text-red-100" : "text-slate-800/70" : "text-slate-500"}`}>{date}</span><strong className={`mt-1 block text-[9px] ${isActive ? isLive ? "text-white" : "text-slate-950" : "text-slate-300"}`}>{meta}</strong><span className={`block text-[7px] ${isActive ? isLive ? "text-red-100/80" : "text-slate-800/60" : "text-slate-600"}`}>{extra}</span></Link>; })}</div></section>;
}
