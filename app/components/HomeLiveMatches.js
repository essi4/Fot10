...
const LEAGUE_RANK = ["UEFA Champions League", "Premier League", "La Liga", "Serie A", "Bundesliga", "Ligue 1", "Eredivisie", "Primeira Liga", "Iranian Pro League", "Persian Gulf Pro League", "Saudi Pro League", "Super Lig", "Brasileirao", "Liga Profesional", "Pro League", "Premiership", "MLS", "Liga MX", "J1 League", "K League 1"];
...
export default function HomeLiveMatches() {
  ...
  const load = async () => { try { const res = await fetch("/api/football/live", { cache: "no-store" }); const json = await res.json(); setMatches(Array.isArray(json.matches) ? filterMajorLeagues(json.matches.filter(live)) : []); } catch {} finally { setLoaded(true); } };
  useEffect(() => { load(); const timer = setInterval(load, 30000); return () => clearInterval(timer); }, []);
  const groups = useMemo(() => { const sorted = [...matches].sort((a, b) => leagueRank(a.league) - leagueRank(b.league)); const iran = sorted.filter((m) => /iran|persian gulf|iranian/i.test(`${m.league} ${m.country}`)); const major = sorted.filter((m) => !iran.includes(m)); return { major, iran }; }, [matches]);
  ...
  return <section className="mt-5"><div className="mb-3 flex items-end justify-between"><div><div className="flex items-center gap-2"><span className="h-2 w-2 animate-pulse rounded-full bg-red-400"/><h2 className="text-sm font-black">بازی‌های زنده</h2></div><p className="mt-1 text-[9px] font-bold text-slate-600">فقط مسابقات لیگ‌های معتبر</p></div><Link href="/matches?live=1" className="text-[9px] font-black text-red-300">همه زنده‌ها ←</Link></div>
    {!matches.length ? <div className="rounded-3xl border border-white/10 bg-white/[.025] px-5 py-7 text-center"><div className="mx-auto grid h-12 w-12 place-items-center rounded-2xl bg-red-500/10 text-red-400"><Radio size={21}/></div><p className="mt-3 text-[11px] font-black">فعلاً بازی زنده‌ای از لیگ‌های معتبر در جریان نیست</p><p className="mt-1 text-[9px] font-bold text-slate-600">لیگ‌های ضعیف و دسته‌های فرعی در این بخش فیلتر شده‌اند.</p></div> : <div className="space-y-4">
      {!!groups.major.length && <div><div className="mb-2 flex items-center gap-2"><span className="text-sm">⭐</span><h3 className="text-[10px] font-black text-slate-200">لیگ‌های معتبر جهان</h3><span className="rounded-full bg-red-400/10 px-2 py-0.5 text-[8px] font-black text-red-300">{groups.major.length}</span></div><div className="grid gap-2.5">{groups.major.map((m) => <MatchCard key={m.id} match={m}/>)}</div></div>}
      {!!groups.iran.length && <div><div className="mb-2 flex items-center gap-2"><span className="text-sm">🇮🇷</span><h3 className="text-[10px] font-black text-slate-200">لیگ‌های معتبر ایران</h3></div><div className="grid gap-2.5">{groups.iran.map((m) => <MatchCard key={m.id} match={m}/>)}</div></div>}
    </div>}
  </section>;
}
