...
import { ExternalLink, Heart, RefreshCw, Newspaper } from "lucide-react";
...
export default function FavoriteTeamNews({ teams = [] }) {
  ...
  useEffect(() => { load(false); }, [teams.join("|")]);
  useEffect(() => {
  ...
  return (
    <section className="glass card p-5" dir="rtl">
      <div className="flex items-center justify-between gap-3 mb-5">
        <div className="flex items-center gap-3"><div className="h-10 w-10 rounded-2xl bg-rose-400/10 grid place-items-center"><Heart size={18} className="text-rose-300" /></div><div><h2 className="font-black">اخبار تیم‌های محبوب</h2><p className="text-[10px] text-slate-500 mt-1">برای هر تیم تا ۵ خبر اختصاصی و تازه</p></div></div>
      ...
      {loading ? <div className="grid gap-3 md:grid-cols-2">{teams.slice(0,5).map((team) => <div key={team} className="rounded-2xl bg-white/[.03] h-40 animate-pulse" />)}</div> : <div className="space-y-5">{teams.slice(0,5).map((team) => <TeamNews key={team} team={team} news={items[team] || []} />)}</div>}
    </section>
  );
}

function TeamNews({ team, news }) {
  return <div className="rounded-2xl border border-white/7 bg-white/[.025] p-3">
    <div className="flex items-center justify-between gap-2 mb-3"><div className="font-black text-sm">⭐ {team}</div><span className="text-[9px] text-slate-600">{fa(news.length)} خبر</span></div>
    {news.length ? <div className="grid gap-2 md:grid-cols-2 lg:grid-cols-3">{news.map((item) => <a key={item.id} href={item.link} target="_blank" rel="noreferrer" className="group rounded-xl border border-white/5 bg-black/10 overflow-hidden hover:bg-white/[.05] transition">
      {item.image ? <img src={`/api/news/image?url=${encodeURIComponent(item.image)}`} alt="" className="h-28 w-full object-cover" loading="lazy" /> : <div className="h-28 bg-white/[.03] grid place-items-center"><Newspaper size={22} className="text-slate-700" /></div>}
      <div className="p-3"><div className="text-[9px] text-emerald-300 font-bold">{item.source} · {ago(item.publishedAt)}</div><h3 className="mt-1.5 text-xs font-black leading-5 line-clamp-2 group-hover:text-emerald-300">{item.title}</h3><div className="mt-2 flex items-center justify-between text-[9px] text-slate-600"><span>خبر اختصاصی تیم</span><ExternalLink size={11} /></div></div>
