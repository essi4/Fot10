...
import { Clock3, ExternalLink, Flame, Globe2, Newspaper, RefreshCw, Search } from "lucide-react";
...
export default function NewsCenter() {
  ...
  const load = async (silent = false) => {
    ...
    try {
      ...
      setAllNews(Array.isArray(json.news) ? json.news : []);
    } catch {
      setFailed(true);
    } finally {
      setLoading(false);
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-[10px] font-bold text-slate-500">ایران، جهان و خبر تیم‌های محبوب؛ در یک جریان زنده</p>
            </div>
          </div>
