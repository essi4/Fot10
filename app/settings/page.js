...
import { Bell, Check, ChevronLeft, Globe2, Heart, Info, LockKeyhole, Moon, Palette, ShieldCheck, SlidersHorizontal, Smartphone, Sparkles, Trophy, Zap } from "lucide-react";
...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    readFavoriteCount();
    const syncFavorites = () => readFavoriteCount();
    window.addEventListener("storage", syncFavorites);
    window.addEventListener("fot10-favorites-changed", syncFavorites);
    window.addEventListener("focus", syncFavorites);
    setReady(true);
    return () => {
      window.removeEventListener("storage", syncFavorites);
      window.removeEventListener("fot10-favorites-changed", syncFavorites);
  ...
  function readFavoriteCount() {
    try {
      ...
      setFavoriteCount(Array.isArray(profile.favorites) ? Math.min(profile.favorites.length, 5) : 0);
  ...
  return <main className="fot-container min-h-screen pb-10">
    ...
    <section className="mt-5 rounded-2xl border border-rose-300/10 bg-rose-400/[.035] p-4"><Link href="/account" className="flex items-center gap-3 group"><div className="grid h-11 w-11 shrink-0 place-items-center rounded-xl border border-rose-300/15 bg-rose-400/10 text-rose-300"><Heart size={18}/></div><div className="min-w-0 flex-1 text-right"><h2 className="text-[11px] font-black">علاقه‌مندی‌ها</h2><p className="mt-1 text-[8px] font-bold text-slate-500">مدیریت همان تیم‌های محبوب حساب من؛ حداکثر ۵ تیم</p></div><div className="text-left"><div className="text-[10px] font-black text-rose-200">{favoriteCount}/۵</div><div className="mt-1 text-[8px] text-slate-600">مدیریت</div></div><ChevronLeft size={16} className="text-slate-600 transition group-hover:text-rose-300"/></Link></section>
    <section className="mt-5 rounded-2xl border border-cyan-300/15 bg-cyan-400/[.05] p-4"><Link href="/ai-lab" className="flex items-center gap-3 group"><div className="grid h-11 w-11 shrink-0 place-items-center rounded-xl border border-cyan-300/20 bg-cyan-400/10 text-cyan-300"><Sparkles size={19}/></div><div className="min-w-0 flex-1 text-right"><h2 className="text-[11px] font-black">آزمایشگاه هوش مصنوعی</h2><p className="mt-1 text-[8px] font-bold text-slate-500">تست چندمدلی FOT10 با AshnaAI</p></div><div className="text-left"><div className="text-[9px] font-black text-cyan-200">AI LAB</div><div className="mt-1 text-[8px] text-slate-600">آزمایشی</div></div><ChevronLeft size={16} className="text-slate-600 transition group-hover:text-cyan-300"/></Link></section>
    <section className="mt-5 rounded-2xl border border-emerald-300/15 bg-emerald-400/[.05] p-4"><Link href="/inspector" className="flex items-center gap-3 group"><div className="grid h-11 w-11 shrink-0 place-items-center rounded-xl border border-emerald-300/20 bg-emerald-400/10 text-emerald-300"><ShieldCheck size={19}/></div><div className="min-w-0 flex-1 text-right"><h2 className="text-[11px] font-black">بازرس ارشد</h2><p className="mt-1 text-[8px] font-bold text-slate-500">۵ کنترل واقعی برای اپ، Live، دیروز، امروز و فردا</p></div><div className="text-left"><div className="text-[9px] font-black text-emerald-200">بررسی</div><div className="mt-1 text-[8px] text-slate-600">Production</div></div><ChevronLeft size={16} className="text-slate-600 transition group-hover:text-emerald-300"/></Link></section>
    <section className="mt-5"><div className="mb-2 px-1"><h2 className="text-xs font-black">ظاهر برنامه</h2><p className="mt-1 text-[8px] text-slate-600">ظاهر مورد علاقه‌ات را انتخاب کن</p></div><div className="rounded-2xl border border-white/10 bg-white/[.035] p-3.5"><div className="flex items-center gap-3"><div className="grid h-11 w-11 shrink-0 place-items-center rounded-xl border border-cyan-300/15 bg-cyan-400/10 text-cyan-300"><Moon size={18}/></div><div className="flex-1 text-right"><div className="text-[11px] font-black">حالت تیره</div><div className="mt-1 text-[8px] text-slate-600">حالت فعلی و مناسب استفاده شبانه</div></div><Toggle value={settings.darkMode} onChange={(v) => set("darkMode", v)} label="حالت تیره"/></div></div><div className="mt-2.5 grid grid-cols-2 gap-2.5"><div className="rounded-2xl border border-emerald-300/15 bg-emerald-400/[.06] p-3.5"><Palette size={16} className="text-emerald-300"/><p className="mt-2 text-[10px] font-black">تم FOT10</p><p className="mt-1 text-[8px] text-slate-600">سبز · تیره · حرفه‌ای</p></div><div className="rounded-2xl border border-violet-300/10 bg-violet-400/[.04] p-3.5"><Globe2 size={16} className="text-violet-300"/><p className="mt-2 text-[10px] font-black">زبان</p><p className="mt-1 text-[8px] text-slate-600">فارسی</p></div></div></section>
