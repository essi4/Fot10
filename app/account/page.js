...
export default function AccountPage() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(async ({ data }) => {
      if (!active) return;
      if (data.user) { setUser(data.user); setEmail(data.user.email || ""); await loadCloudProfile(data.user.id); }
      setLoading(false);
    ...
    const { data: listener } = supabase.auth.onAuthStateChange(async (_event, session) => {
      ...
      setUser(nextUser);
      if (nextUser) { setEmail(nextUser.email || ""); await loadCloudProfile(nextUser.id); }
    });
    return () => { active = false; listener.subscription.unsubscribe(); };
  }, [supabase]);

  function loadLocalProfile() { try { return JSON.parse(localStorage.getItem("fot10-profile") || "{}"); } catch { return {}; } }
  ...
  async function loadCloudProfile(userId) {
    ...
    try {
      ...
      if (profileError) throw profileError; if (prefsError) throw prefsError; if (favError) throw favError;
    ...
    } catch (e) { setError(e?.message || "خطا در دریافت اطلاعات حساب"); } finally { setBusy(false); }
  }

  function persistGuest(next = {}) {
    const data = { name, favorites, favoriteLeagues, notifications, ...next };
    localStorage.setItem("fot10-profile", JSON.stringify(data)); setSaved(true); window.setTimeout(() => setSaved(false), 1400);
  }

  async function saveCloud(next = {}) {
    if (!supabase || !user) { persistGuest(next); return; }
    setBusy(true); setError("");
    try {
      ...
      if (profileError) throw profileError; if (prefsError) throw prefsError;
      const { error: deleteError } = await supabase.from("fot10_favorites").delete().eq("user_id", user.id).in("item_type", ["team", "league"]);
      if (deleteError) throw deleteError;
      const rows = [...nextTeams.map(item_name => ({ user_id: user.id, item_type: "team", item_name })), ...nextLeagues.map(item_name => ({ user_id: user.id, item_type: "league", item_name }))];
      if (rows.length) { const { error: favoriteError } = await supabase.from("fot10_favorites").insert(rows); if (favoriteError) throw favoriteError; }
      setSaved(true); window.setTimeout(() => setSaved(false), 1400);
    } catch (e) { setError(e?.message || "ذخیره‌سازی ابری انجام نشد"); } finally { setBusy(false); }
  }

  async function sendOtp() {
    if (!supabase) return setError("اتصال Supabase در محیط اجرا تنظیم نشده است.");
    const value = email.trim().toLowerCase(); if (!value || !value.includes("@")) return setError("ایمیل معتبر وارد کن.");
    setBusy(true); setError(""); setMessage("");
    const { error: authError } = await supabase.auth.signInWithOtp({ email: value, options: { shouldCreateUser: true } });
    setBusy(false); if (authError) return setError(authError.message); setOtpSent(true); setMessage("کد ورود به ایمیلت ارسال شد. کد ۶ رقمی را وارد کن.");
  }

  async function verifyOtp() {
    if (!supabase) return; if (!/^\d{6}$/.test(otp.trim())) return setError("کد ورود باید ۶ رقمی باشد.");
    setBusy(true); setError("");
    const { data, error: authError } = await supabase.auth.verifyOtp({ email: email.trim().toLowerCase(), token: otp.trim(), type: "email" });
    setBusy(false); if (authError) return setError(authError.message);
    if (data.user) { setUser(data.user); setMessage("ورود با موفقیت انجام شد ✓"); await loadCloudProfile(data.user.id); }
  }
  ...
  return (
    <main className="fot-shell min-h-screen"><div className="fot-container pb-10 space-y-4">
      <header className="flex items-center justify-between pt-1"><a href="/" className="glass touch-target rounded-2xl px-4 py-2 text-xs font-bold">← خانه</a><div className="text-right"><div className="text-[10px] text-slate-500">FOT10</div><h1 className="text-xl font-black">حساب من</h1></div></header>
      <section className="glass card p-5 relative overflow-hidden"><div className="absolute -left-16 -top-16 h-40 w-40 rounded-full bg-emerald-400/10 blur-3xl"/><div className="relative flex items-center gap-4"><div className="h-16 w-16 rounded-[22px] bg-gradient-to-br from-emerald-300 to-cyan-500 text-slate-950 grid place-items-center shadow-xl shadow-emerald-500/10"><UserRound size={28}/></div><div className="min-w-0 flex-1"><div className="text-[10px] text-emerald-300 font-bold">{user ? "حساب متصل FOT10" : "شخصی‌سازی FOT10"}</div><div className="text-xl font-black mt-1 truncate">{name || (user?.email || "مهمان FOT10")}</div><div className="text-[10px] text-slate-500 mt-1">{user ? user.email : "ورود اختیاری • اطلاعات مهمان روی گوشی ذخیره می‌شود"}</div></div></div><div className="grid grid-cols-3 gap-2 mt-5"><Mini n={favorites.length + favoriteLeagues.length} t="مورد علاقه"/><Mini n="۲۴/۷" t="پوشش فوتبال"/><Mini n={user ? "SYNC" : "GUEST"} t="وضعیت حساب"/></div></section>
      {!user && <section className="glass card p-5"><div className="flex items-center gap-3 mb-4"><LogIn size={18} className="text-emerald-400"/><div><h2 className="font-black">ورود امن</h2><p className="text-[10px] text-slate-500 mt-1">بدون رمز عبور؛ کد یک‌بارمصرف به ایمیل ارسال می‌شود.</p></div></div><div className="flex gap-2"><input value={email} onChange={e => setEmail(e.target.value)} dir="ltr" type="email" inputMode="email" className="glass min-w-0 flex-1 rounded-2xl py-3.5 px-4 outline-none focus:ring-2 focus:ring-emerald-400/20 text-sm" placeholder="you@example.com"/><button disabled={busy} onClick={sendOtp} className="rounded-2xl bg-emerald-400 text-slate-950 px-4 font-black text-xs disabled:opacity-50">ارسال کد</button></div>{otpSent && <div className="mt-3 flex gap-2"><input value={otp} onChange={e => setOtp(e.target.value.replace(/\D/g, "").slice(0,6))} dir="ltr" inputMode="numeric" maxLength={6} className="glass flex-1 rounded-2xl py-3.5 px-4 text-center tracking-[.5em] outline-none" placeholder="••••••"/><button disabled={busy} onClick={verifyOtp} className="rounded-2xl bg-white text-slate-950 px-4 font-black text-xs disabled:opacity-50">تأیید</button></div>}</section>}
