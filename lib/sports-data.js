...
export async function getLeagueByCountry(country, preferredName) {
  ...
  const response = await apiFootball("/leagues", { country, current: true, type: "league" });
...
export async function getLeagueCurrentSeason(league) {
  if (DEMO_MODE) return null;
  const response = await apiFootball("/leagues", { id: league, current: true });
  const seasons = response[0]?.seasons || [];
  return seasons.find((season) => season.current)?.year || seasons[0]?.year || null;
}
...
export async function getPlayerById(id, season) {
  if (DEMO_MODE) return null;
  const response = await apiFootball("/players", { id, season });
