...
function makeApiError(status, errors, headers) {
  ...
  if (status === 401 || status === 403) return new SportsApiError("کلید API فوتبال معتبر نیست یا دسترسی آن رد شده است.", "AUTH_ERROR", { status });
  if (status === 429) return new SportsApiError("سقف درخواست API فوتبال پر شده است؛ کمی بعد دوباره تلاش کنید.", "RATE_LIMIT", { status });
  if (status >= 500) return new SportsApiError("سرویس API فوتبال موقتاً پاسخ نمی‌دهد.", "UPSTREAM_ERROR", { status });
  return new SportsApiError(`خطای API فوتبال: ${detail}`, "API_ERROR", { status, dailyRemaining: headers?.get("x-ratelimit-requests-remaining") || null, minuteRemaining: headers?.get("X-RateLimit-Remaining") || null });
}
...
async function apiFootball(path, params = {}, options = {}) {
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    try {
      const response = await fetch(url, { method: "GET", headers: { "x-apisports-key": API_KEY, Accept: "application/json" }, ...(options.revalidate ? { next: { revalidate: options.revalidate } } : { cache: "no-store" }) });
      const text = await response.text();
      let payload = null;
      try { payload = text ? JSON.parse(text) : {}; } catch { throw new SportsApiError(`API-Football returned invalid JSON (HTTP ${response.status}).`, "INVALID_RESPONSE", { status: response.status }); }
      const errors = payload?.errors;
      const hasErrors = errors && (Array.isArray(errors) ? errors.length : Object.keys(errors).length);
      if (!response.ok || hasErrors) {
        const error = makeApiError(response.status, errors, response.headers); lastError = error;
      ...
      if (error instanceof SportsApiError) { if (error.code === "UPSTREAM_ERROR" && attempt === 0) { await new Promise((resolve) => setTimeout(resolve, 700)); continue; } throw error; }
      lastError = new SportsApiError(`API-Football network error: ${error?.message || "request failed"}`, "NETWORK_ERROR");
...
export async function getLeagueByCountry(country, preferredName) {
  ...
  if (!response.length) response = await apiResponse("/leagues", { country, type: "league" }, { revalidate: 3600 });
...
export async function getLeagueCurrentSeason(league) {
  ...
  const seasons = response[0]?.seasons || []; const withStandings = seasons.filter((season) => season?.coverage?.standings);
...
export async function getMatches({ date, live = false, league, season, team } = {}) {
  ...
  let response = await apiResponse("/fixtures", { ...(live ? { live: "all" } : date ? { date } : {}), ...baseParams }, { revalidate: live ? 15 : 60 });

  // Some competition feeds occasionally lag on the exact-date filter. When that happens,
  // retry the same day as an explicit range before giving the UI a false empty state.
  if (!live && date && !response.length && league && season) {
    response = await apiResponse("/fixtures", { ...baseParams, from: date, to: date }, { revalidate: 60 });
  }
...
export async function getPlayerById(id, season) {
  ...
  for (const year of seasons) { const response = await apiResponse("/players", { id, season: year }, { revalidate: 3600 }); if (response[0]) return { ...response[0], _season: year }; }
  return null;
}
...
export async function getFixtureDetails(fixtureId) { if (DEMO_MODE) return null; const response = await apiResponse("/fixtures", { id: fixtureId }, { revalidate: 15 }); return response[0] || null; }
export async function getFixtureEvents(fixtureId) { if (DEMO_MODE) return []; return apiResponse("/fixtures/events", { fixture: fixtureId }, { revalidate: 15 }); }
export async function getFixtureStatistics(fixtureId) { if (DEMO_MODE) return []; return apiResponse("/fixtures/statistics", { fixture: fixtureId }, { revalidate: 15 }); }
