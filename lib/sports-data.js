...
async function apiFootball(path, params = {}, options = {}) {
  ...
  // Quota protection: one upstream request by default. Retrying blindly after an
  // upstream error can consume another request without adding user value.
...
export async function getPlayerById(id, season) { if (DEMO_MODE) return null; const seasons = season ? [Number(season)] : [new Date().getUTCFullYear(), new Date().getUTCFullYear() - 1, new Date().getUTCFullYear() - 2]; for (const year of seasons) { const response = await apiResponse("/players", { id, season: year }, { revalidate: 3600 }); if (response[0]) return { ...response[0], _season: year }; } return null; }
