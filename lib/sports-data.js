...
export function getSportsDataConfig() { return { provider: process.env.SPORTS_DATA_PROVIDER || "api-football", hasApiKey: Boolean(process.env.SPORTS_API_KEY), demoMode: DEMO_MODE }; }
...
async function apiFootball(path, params = {}) {
  ...
  const response = await fetch(url, { headers: { "x-apisports-key": process.env.SPORTS_API_KEY }, cache: "no-store" });
...
export async function getMatches({ date, live = false, league, season, team } = {}) {
  if (DEMO_MODE) return [];
  const response = await apiFootball("/fixtures", { ...(live ? { live: "all" } : date ? { date } : {}), league, season, team, timezone: "Asia/Tehran" });
...
export async function getTeamBySearch(search) { if (DEMO_MODE) return null; const response = await apiFootball("/teams", { search }); return response[0] || null; }
...
export async function getStandings(league, season) { if (DEMO_MODE) return []; return apiFootball("/standings", { league, season }); }
