...
export async function getMatches({ date, live = false, league, season, team } = {}) {
  if (DEMO_MODE) return [];
  const response = await apiFootball("/fixtures", { ...(live ? { live: "all" } : { date }), league, season, team, timezone: "Asia/Tehran" });
...
export async function getTeamFixtures(team, season) {
  if (DEMO_MODE) return [];
  return getMatches({ team, season, date: undefined });
}
