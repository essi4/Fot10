...
export function getSportsDataConfig() {
  return { provider: process.env.SPORTS_DATA_PROVIDER || "api-football", hasApiKey: Boolean(API_KEY), demoMode: DEMO_MODE };
}
...
async function apiFootball(path, params = {}) {
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    try {
      const response = await fetch(url, { method: "GET", headers: { "x-apisports-key": API_KEY, Accept: "application/json" }, cache: "no-store" });
...
export async function getTeamBySearch(search) { if (DEMO_MODE) return null; const response = await apiResponse("/teams", { search }); return response[0] || null; }
export async function getTeamById(team) { if (DEMO_MODE) return null; const response = await apiResponse("/teams", { id: team }); return response[0] || null; }
