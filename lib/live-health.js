...
async function checkPrimary() {
  ...
  if (circuit.open) {
    return { ok: false, code: "CIRCUIT_OPEN", message: `API-Football circuit open: ${circuit.reason || "UPSTREAM_FAILURE"}`, provider: "api-football", circuit };
  }
  ...
  try {
    ...
    const failure = { code: error?.name === "TimeoutError" ? "TIMEOUT" : "NETWORK_ERROR", message: error?.message || "API-Football request failed" };
    const circuitState = recordApiFootballFailure(failure);
    return { ok: false, ...failure, provider: "api-football", latencyMs: Date.now() - started, circuit: circuitState };
  }
  ...
  if (!response.ok || (payload?.errors && Object.keys(payload.errors).length)) {
    ...
    const message = typeof errors === "string" ? errors : errors && Object.values(errors).join(" | ") || `HTTP ${response.status}`;
    const code = /account\s+is\s+suspended|your\s+account\s+is\s+suspended/i.test(message)
      ? "SUSPENDED"
      : response.status === 401 || response.status === 403
        ? "AUTH_ERROR"
        : response.status === 429
          ? "RATE_LIMIT"
          : /firewall|blocked|forbidden/i.test(message)
            ? "FIREWALL"
            : response.status >= 500
              ? "UPSTREAM_ERROR"
              : "API_ERROR";
    const circuitState = recordApiFootballFailure({ code, message, details: { status: response.status } });
    return { ok: false, code, httpStatus: response.status, message, provider: "api-football", latencyMs, rateLimit: { dailyRemaining, minuteRemaining }, circuit: circuitState };
  }

  const matches = Array.isArray(payload?.response) ? payload.response : [];
  const liveMatches = matches.filter(liveStatus);
  const circuitState = recordApiFootballSuccess();
  return {
    ok: true,
...
export async function getLiveHealth({ deep = false, force = false } = {}) {
  ...
  inFlight = (async () => {
    ...
    // Even when the primary circuit is open, probe the fallback so health reflects the actual recovery path.
    const fallback = primary.ok ? { checked: false, reason: "primary_ok" } : await checkFallback();
    const serviceOk = primary.ok || fallback.ok;
    const health = {
      // The Live service is healthy when either the primary provider or the working fallback can serve it.
      // Primary provider failure remains fully visible below; this is a degraded-but-available state, not a hidden error.
      ok: serviceOk,
      checkedAt,
      service: "fot10-live-api",
      primary,
      fallback,
      circuit: getApiFootballCircuitState(),
      diagnosis: primary.ok ? "provider_ok" : fallback.ok ? "degraded_fallback_ok" : "all_providers_failed",
